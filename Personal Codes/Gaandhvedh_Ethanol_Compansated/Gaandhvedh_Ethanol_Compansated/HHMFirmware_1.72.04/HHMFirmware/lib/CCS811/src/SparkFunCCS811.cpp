...
CCS811Core::CCS811_Status_e CCS811::readRawData( void )
{
	uint8_t data[2];
	CCS811Core::CCS811_Status_e returnError = multiReadRegister(CSS811_RAW_DATA, data, 2);     
...
//Returns the Firmware Boot Version, running in CCS811
uint16_t CCS811::getBootVer( void ){
	
	uint8_t data[2];
	CCS811Core::CCS811_Status_e returnError = multiReadRegister(CSS811_FW_BOOT_VERSION, data, 2);

	unsigned int boot_ver = ((uint16_t)data[0] << 8) | data[1];
	if( returnError != CCS811_Stat_SUCCESS )
	{
		return 0;
	}
...
//Returns the Firmware App Version, running in CCS811
uint16_t CCS811::getAppVer( void ){

	uint8_t data[2];
	CCS811Core::CCS811_Status_e returnError = multiReadRegister(CSS811_FW_BOOT_VERSION, data, 2);
 returnError = multiReadRegister(CSS811_FW_APP_VERSION, data, 2);

	unsigned int app_ver = ((uint16_t)data[0] << 8) | data[1];
	if( returnError != CCS811_Stat_SUCCESS )
	{
		return 0;
	}
...
//Returns the Hardware version of CCS811
uint16_t CCS811::getHWVer( void ){

	uint8_t data[1];
	CCS811Core::CCS811_Status_e returnError = multiReadRegister(CSS811_HW_VERSION, data, 1);

	unsigned int HW_ver = (uint8_t)data[0] ;
	if( returnError != CCS811_Stat_SUCCESS )
	{
		return 0;
	}
...
uint16_t CCS811::getRawData( void )
{
	return rawData;
}
