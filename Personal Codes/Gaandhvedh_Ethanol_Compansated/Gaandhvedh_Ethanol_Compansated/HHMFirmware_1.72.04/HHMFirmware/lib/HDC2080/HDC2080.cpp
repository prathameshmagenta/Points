...
float HDC2080::readLowHumidityThreshold(void)
{
	...
	if(readReg(HUMID_THR_L, regContents)){
		f = regContents;
		f = f * 100.0f / 256.0f;
	}
	return f;
...
float HDC2080::readHighHumidityThreshold(void)
{
	...
	if(readReg(HUMID_THR_H, regContents)){
		f = regContents;
		f = f * 100.0f / 256.0f;
	}
	return f;
...
float HDC2080::readLowTempThreshold(void)
{
	...
	if(readReg(TEMP_THR_L, regContents)){
		f = regContents;
...
float HDC2080::readHighTempThreshold(void)
{
	...
	if(readReg(TEMP_THR_H, regContents)){
		f = regContents;
		f = (f * 165.0f / 256.0f)-40.0f;
	}
	return f;
...
{ 
	...
	readReg(MEASUREMENT_CONFIG, configContents);
...
{
	uint8_t regContents;
	readReg(INTERRUPT_DRDY, regContents);
	return regContents;
...
float HDC2080::readMaxTemp(void)
{
	...
	if(readReg(TEMP_MAX, regContents)){
		f = regContents;
		f = (f * 165.0f / 256.0f)-40.0f;
	}
	return f;
...
float HDC2080::readMaxHumidity(void)
{
	...
	if(readReg(HUMID_MAX, regContents)){
		f = regContents;
		f = (f / 256.0f)*100.0f;
	}
	return f;
...
{
	uint8_t regContents;
	readReg(INTERRUPT_CONFIG,regContents);
...
// Read Temp & RH from HDC2080
void HDC2080::readHDC2080TRH(void)
{
	uint8_t byte[4], i;

	openReg(DATA_REG);
	Wire.requestFrom(_addr, 4); 		// Request 4 bytes from open register
	if (Wire.available()){
	...
	humidity = ((byte[3]<<8) | byte[2]);
	humidity = (humidity/65536.0f)*100.0f;
}			
		
// Returns the temperature in degrees C		
float HDC2080::getTemp(void)
{
	return temp;
}					

// Returns the relative humidity
float HDC2080::getHumidity(void)
{
	return humidity;
}		

bool HDC2080::readTempCalibration(uint8_t &data){

	return readReg(TEMP_OFFSET_ADJUST, data);		
}

void HDC2080::calibrateTemp(uint8_t value){

	writeReg(TEMP_OFFSET_ADJUST, value);	
}

bool HDC2080::readHumCalibration(uint8_t &data){

	return readReg(HUM_OFFSET_ADJUST, data);		
}

void HDC2080::calibrateHum(uint8_t value){
