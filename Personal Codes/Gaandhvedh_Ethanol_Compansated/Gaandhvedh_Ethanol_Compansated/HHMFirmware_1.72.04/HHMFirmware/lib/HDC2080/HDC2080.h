...
#define TEMP_CALI_BIT_0		0.16f
#define TEMP_CALI_BIT_1		0.32f
#define TEMP_CALI_BIT_2		0.64f
#define TEMP_CALI_BIT_3		1.28f
#define TEMP_CALI_BIT_4		2.58f
#define TEMP_CALI_BIT_5		5.16f
#define TEMP_CALI_BIT_6		10.32f
#define TEMP_CALI_BIT_7		20.62f

#define HUM_CALI_BIT_0		0.2f
#define HUM_CALI_BIT_1		0.4f
#define HUM_CALI_BIT_2		0.8f
#define HUM_CALI_BIT_3		1.6f
#define HUM_CALI_BIT_4		3.1f
#define HUM_CALI_BIT_5		6.3f
#define HUM_CALI_BIT_6		12.5f
#define HUM_CALI_BIT_7		25.0f
...
//		float readHumidity(void);				// Returns the relative humidity
		...
		void readHDC2080TRH(void);				// Read Temp & RH from HDC2080
		float getTemp(void);					// Returns the temperature in degrees C
		float getHumidity(void);				// Returns the relative humidity
		...
		bool readTempCalibration(uint8_t &data);
		void calibrateTemp(uint8_t value);
		bool readHumCalibration(uint8_t &data);
		void calibrateHum(uint8_t value);
		
	private:
		int _addr; 									// Address of sensor 
		float temp;									// Temp value
		float humidity;								// Humidity Value
		void openReg(uint8_t reg); 	    			// Points to a given register
		bool readReg(uint8_t reg, uint8_t &data);	// Reads a given register, returns 1 byte
