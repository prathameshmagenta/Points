...
void All_Init(void)
{
  ...
  // Serial.printf("CORE ID: %s \n", ("CG_POINT_" + String(uint32_t(Read_EEPROM_Float(EEPROM_CHIP_ID_ADD)))).c_str());
  Serial.printf("CPO ID: %s", ("CG_POINT_" + Read_EEPROM_String(EEPROM_CHARGER_ID_ADD)).c_str());
}
...
void setup()
{
  ...
  xTaskCreateUniversal(&LED_Thread, "LED_Thread", 1024, NULL, 1, &LED_HANDLE, 0);
