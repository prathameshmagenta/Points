...
void RX_Json_Packet_Field_Identifier(void)
{
    ...
    if (isnan(Transaction_Rate) || isinf(Transaction_Rate))
        Transaction_Rate = Read_EEPROM_Float(EEPROM_ENERGY_RATE_ADD);

    if (Rate_Update == "Yes" || Rate_Update == "YES" || Rate_Update == "yes")
