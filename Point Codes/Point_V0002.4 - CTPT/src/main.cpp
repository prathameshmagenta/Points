...
void Main_Thread(void *pvParam)
{
  while (1)
  {
    ...
    while (!device_connect_status())
      ;
    Charger_State = 2;
    set_ble_name(String("CGPOINT" + Read_EEPROM_String(EEPROM_CHARGER_ID_ADD) + "_O"));
...
void All_Init(void)
{
  ...
  Serial.printf("CPO ID: %s\n", String("CGPOINT" + Read_EEPROM_String(EEPROM_CHARGER_ID_ADD)).c_str());
...
void setup()
{
  ...
  xTaskCreateUniversal(&LED_Thread, "LED_Thread", 4096, NULL, 2, NULL, 0);
  xTaskCreateUniversal(&JSON_Frame_Thread, "JSON_Frame_Thread", 4096, NULL, 2, &JSON_FRAME_READ_HANDLE, 1);
  xTaskCreateUniversal(&Read_Meter_Values_Thread, "READ_Meter_Values", 4096, NULL, 2, &READ_METER_VALUE_HANDLE, 1);
